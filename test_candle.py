...
async def test_get_candle():
    ...
    try:
        ...
        # Try different timeframes and parameter formats
        tfs = ['5m', '15m', 'M5', 'M15', 'm5', 'm15']
        for tf in tfs:
            try:
                print(f"Testing tf: {tf}")
                candle = await connection.get_candle('EURUSD', tf, 1)
                print(f"Success for {tf}: {candle}")
                break
